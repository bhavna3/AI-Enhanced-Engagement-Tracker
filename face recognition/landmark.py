...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"screenshots1/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive:
        ...
        if (datetime.now() - last_save_time) > timedelta(seconds=30):
            df.to_excel('attendance_with_attention(7).xlsx', index=False)
            print("Data saved to 'attendance_with_attention(7).xlsx'")
    ...
    if not df.empty:
        df.to_excel('attendance_with_attention(7).xlsx', index=False)
        print("Final data saved to 'attendance_with_attention(7).xlsx'")
