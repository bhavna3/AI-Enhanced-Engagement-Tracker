...
if not os.path.exists("screenshots(5)"):
    os.makedirs("screenshots(5)")
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    # Save screenshot
                    screenshot_filename = f"screenshots(5)/__{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                    cv.imwrite(screenshot_filename, frame)  # Save the screenshot

                    # Log a new entry for   with screenshot
        ...
        if not recognized:
            # If the face is not recognized, label it as 'Not  '
            cv.putText(frame, 'Not Bhavana ', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                       (255, 0, 0), 2, cv.LINE_AA)
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('_attendance_with_screenshots(5).xlsx', index=False)
            print("DataFrame saved to '_attendance_with_screenshots(5).xlsx'")
    ...
    if not df.empty:
        df.to_excel('_attendance_with_screenshots(5).xlsx', index=False)
        print("Final attendance and screenshots saved to '_attendance_with_screenshots(5).xlsx'.")
