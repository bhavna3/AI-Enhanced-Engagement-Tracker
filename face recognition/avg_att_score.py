...
p = r"C:\Users\saikeerthana\OneDrive\Desktop\AI engagement tracker\face_recognition\facial-landmarks-recognition\shape_predictor_68_face_landmarks.dat"
...
if not os.path.exists("screenshots(8)"):
    os.makedirs("screenshots(8)")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"screenshots(8)/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive == 'Yes':
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('attendance_with_attention_score(8).xlsx', index=False)
            print("DataFrame saved to 'attendance_with_attention_score(8).xlsx'")
    ...
    if not df.empty:
        df.to_excel('attendance_with_attention_score(8).xlsx', index=False)
        print("Final attendance with attentiveness saved to 'attendance_with_attention_score(8).xlsx'.")
